...
//    String inputFile = "/Users/admin/OneDrive - Platform 3 Solutions LLC/projects/cmod/data/dev/approach2/spoolfile.txt";
    String inputFile = "/Users/admin/OneDrive - Platform 3 Solutions LLC/projects/tmb/docs/manual/hats-loan-api-app/inputs/0000000003.txt";

    private void start() throws Exception {
        byte[] bytes = Files.readAllBytes(new File(inputFile).toPath());
//        String newPage = "31da";//FOR CMOD
        String newPage = "da44617465207072696E7465643A"; // FOR HATSLOAN
        String hexValue = "";
        String content = "";
        String zeros = "00";
        String hexString = "";
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            if (i >= 15) {
                ...
                for (int j=i-15;j<i;j++) {
                    hexValue += Integer.toHexString(bytes[j]);
                }
                if (hexValue.equalsIgnoreCase(newPage)) {
                    pageCounter++;
                }
            }
            if (Integer.toHexString(bytes[i]).equalsIgnoreCase("a")) {
...
//        maxWidth = 1400;
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            if (i >= 15) {
                ...
                if (hexValue.equalsIgnoreCase(newPage)) {
                    byte[] newBytes = javax.xml.bind.DatatypeConverter.parseHexBinary(content.substring(0, content.length() - 28));
                    document.add(new Paragraph(new String(newBytes), font));
                    content = content.substring(content.length() - 28);
                    document.newPage();
                }
            }
        }
